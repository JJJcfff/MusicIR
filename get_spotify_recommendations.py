...
def get_spotify_recommendations(seed_tracks, num_tracks=10):
    sp = auth()
    related_tracks = []
    for track_id in tqdm.tqdm(seed_tracks, desc='Getting recommendations'):
        ...
        related_tracks.extend([{'spotify_id' : track['id'], 'name' : track['name']} for track in results['tracks']])
    for track in related_tracks:
        if not download_preview(track['spotify_id']):
